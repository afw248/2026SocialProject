...
namespace ChangJun.Judge
{
    ...
    ///
    /// 판정 순서:
    ///   1. 손님 주문 메뉴 재료와 일치하면 그 메뉴를 채택 (동일 재료 조합 메뉴 충돌 해소)
    ///   2. 아니면 레시피북에서 탐색 — 없으면 → WrongRecipe (오조리)
    ///   3. 유효 메뉴는 있으나 금기 재료가 포함되면 → TabooViolation (금기위반)
    ///   4. 레시피 일치 AND 금기 없음 AND 주문 일치 → Success
    /// </summary>
    public static class RecipeJudge
    {
        ...
        {
            matched = ResolveMatchedMenu(selected, customer, matcher);
            ...
            if (customer != null)
            {
                ...
                {
                    if (ing != null && (ing.forbiddenIn & customer.diet) != 0)
        ...
        /// <summary>
        /// 동일 재료 조합이 여러 메뉴에 있을 때(예: M11/M24/M25) 손님 주문을 우선한다.
        /// </summary>
        private static MenuRecipeSO ResolveMatchedMenu(
            IReadOnlyList<IngredientSO> selected,
            CraftCustomerSO customer,
            IRecipeMatcher matcher)
        {
            if (customer?.requiredMenu != null && MatchesMenu(selected, customer.requiredMenu))
                return customer.requiredMenu;

            return matcher?.Match(selected);
        }

        private static bool MatchesMenu(IReadOnlyList<IngredientSO> selected, MenuRecipeSO menu)
        {
            if (selected == null || menu?.ingredientCodes == null) return false;
            ...
            if (selectedSet.Count == 0) return false;
            if (selectedSet.Count != menu.ingredientCodes.Length) return false;
            return selectedSet.SetEquals(menu.ingredientCodes);
        }
    }
}
